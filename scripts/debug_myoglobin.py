...
def run_mbn_experiment():
    ...
    # 2. Load Myoglobin Data
    data_path = os.path.join(base, "experiments", "sandbox", "1mbn_ca.pt")
    if not os.path.exists(data_path):
        print("Myoglobin data not found!")
        return

    try:
        x_gt = torch.load(data_path, weights_only=False)
    except:
        x_gt = torch.load(data_path)
    ...
    print(f"Loaded Myoglobin: {n_atoms} atoms")
    ...
    # 4. Reconstruct (Alpha=1.0, Scale=1.59 - Standard Params)
    ...
    print(f"Myoglobin (OOD) RMSD: {rmsd:.4f} Angstroms")
    ...
    for i, (title, dim1, dim2) in enumerate(views):
        ...
        ax.scatter(x_gt_np[:, dim1], x_gt_np[:, dim2], c='red', s=20, alpha=0.6, label='Myoglobin GT')
        ax.scatter(x_rec_aligned[:, dim1], x_rec_aligned[:, dim2], c='purple', s=20, alpha=0.6, label='Reconstruction') # Purple for Mbn
    ...
    plt.suptitle(f"Generalization Test: Myoglobin (1MBN)\nRMSD: {rmsd:.2f} Å (Out-of-Distribution - UNSEEN)", fontsize=14)
    plt.tight_layout()
    
    save_path = os.path.join(base, "experiments", "sandbox", "mbn_result.png")
